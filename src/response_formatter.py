...
class ResponseFormatter:
    ...
    async def _format_analysis_response(
        ...
        """Format analysis response with structured analysis content."""

        sections = []

        if not results:
            return [{"title": "No Data Found", "content": "Unable to find data for analysis.", "type": "message"}]
        ...
        if key_insights:
            sections.append({
                ...
                "type": "insights",
                "items": [insight for insight in key_insights if insight]
            })
        ...
        if len(results) > 5:
            sections.append({
                ...
                "content": self._create_snippet(results[5:10]),
    ...
    def _create_comparison_table(self, entities: List[str], results: List[HybridSearchResult]) -> str:
        """Create a comparison table for entities."""
        if len(entities) < 2:
            return "Not enough entities to compare."

        comparison_data = f"Comparing {entities[0]} vs {entities[1]}:\n\n"

        # Extract relevant information for each entity
        entity1_info = []
        entity2_info = []

        for result in results:
            content_lower = result.content.lower()
            if entities[0].lower() in content_lower:
                entity1_info.append(self._create_snippet(result.content))
            if entities[1].lower() in content_lower:
                entity2_info.append(self._create_snippet(result.content))

        comparison_data += f"**{entities[0]}:**\n"
        comparison_data += "\n".join(entity1_info[:2]) if entity1_info else "No specific information found."
        comparison_data += f"\n\n**{entities[1]}:**\n"
        comparison_data += "\n".join(entity2_info[:2]) if entity2_info else "No specific information found."

        return comparison_data

    def _create_tutorial_intro(self, query: str, results: List[HybridSearchResult]) -> str:
        """Create tutorial introduction."""
        if not results:
            return f"This tutorial will guide you through {query}."

        intro_parts = [f"This tutorial covers {query} based on the following resources:"]

        for result in results:
            intro_parts.append(f"- {result.title} ({result.source_name})")

        intro_parts.append("\nLet's get started with the step-by-step process.")

        return "\n".join(intro_parts)

    def _analyze_problem(self, query: str, results: List[HybridSearchResult]) -> str:
        """Analyze the problem from query and results."""
        analysis_parts = [f"Problem Analysis for: {query}\n"]

        if not results:
            analysis_parts.append("Unable to find specific information about this problem.")
            return "\n".join(analysis_parts)

        # Look for error patterns or problem indicators
        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['error', 'problem', 'issue', 'troubleshoot', 'fix']):
                analysis_parts.append(f"From {result.source_name}:")
                analysis_parts.append(self._create_snippet(result.content))
                break

        if len(analysis_parts) == 1:
            analysis_parts.append("Based on available information, this appears to be a common issue.")
            analysis_parts.append("Please review the solutions below for potential fixes.")

        return "\n".join(analysis_parts)

    def _extract_comparison_entities(self, query: str) -> List[str]:
        """Extract entities being compared from query text."""
        # Simple extraction - look for "vs", "versus", "compared to" patterns
        comparison_words = ['vs', 'versus', 'compared to', 'compare', 'difference between']
        entities = []

        query_lower = query.lower()
        for word in comparison_words:
            if word in query_lower:
                # Split around the comparison word and extract potential entities
                parts = query_lower.split(word)
                if len(parts) >= 2:
                    # Extract last word from first part and first word from second part
                    entity1 = parts[0].strip().split()[-1] if parts[0].strip() else ""
                    entity2 = parts[1].strip().split()[0] if parts[1].strip() else ""
                    if entity1 and entity2:
                        entities.extend([entity1, entity2])
                        break

        return entities[:2]  # Return maximum 2 entities

    def _extract_pros_cons(self, entities: List[str], results: List[HybridSearchResult]) -> str:
        """Extract pros and cons for comparison."""
        if len(entities) < 2:
            return "Insufficient data for pros and cons analysis."

        pros_cons = f"Pros and Cons Analysis:\n\n"

        for entity in entities[:2]:
            pros_cons += f"**{entity.title()}:**\n"
            ...
            for result in results:
                ...
                if entity.lower() in content_lower:
                    ...
                    if any(word in content_lower for word in ['advantage', 'benefit', 'pro', 'good', 'better']):
                        pros.append(self._create_snippet(result.content))
                    if any(word in content_lower for word in ['disadvantage', 'drawback', 'con', 'limitation', 'worse']):
                        cons.append(self._create_snippet(result.content))

            if pros:
                pros_cons += f"Pros: {pros[0]}\n"
            if cons:
                pros_cons += f"Cons: {cons[0]}\n"
            pros_cons += "\n"

        return pros_cons

    def _create_general_comparison(self, results: List[HybridSearchResult]) -> str:
        """Create general comparison when specific entities aren't identified."""
        if not results:
            return "No comparison data available."

        return f"General comparison based on available information:\n\n{self._create_overview(results[:3])}"

    def _extract_prerequisites(self, results: List[HybridSearchResult]) -> List[str]:
        """Extract prerequisites from tutorial content."""
        prerequisites = []

        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['prerequisite', 'requirement', 'before', 'need', 'install']):
                # Extract sentences with prerequisite keywords
                sentences = result.content.split('.')
                for sentence in sentences:
                    if any(keyword in sentence.lower() for keyword in ['prerequisite', 'requirement', 'install']):
                        prerequisites.append(sentence.strip())
                        break

        # Default prerequisites if none found
        if not prerequisites:
            prerequisites = ["Basic understanding of the topic", "Development environment setup"]

        return prerequisites[:3]  # Limit to 3 prerequisites

    def _extract_tutorial_steps(self, results: List[HybridSearchResult]) -> List[str]:
        ...
        if not steps:
            steps = [
                "1. Review the documentation and requirements",
    ...
    def _extract_code_examples(self, results: List[HybridSearchResult]) -> str:
        ...
        for result in results:
            ...
            if '`' in result.content:
                ...
                for code in inline_code:
                    if len(code) > 5:  # Only meaningful code snippets
                        code_examples.append(f"`{code}`")

        if not code_examples:
            return "No code examples found in the available content."

        return "\n\n".join(code_examples[:3])  # Limit to 3 examples

    def _extract_additional_resources(self, results: List[HybridSearchResult]) -> str:
        """Extract additional resources and references."""
        resources = []

        for result in results:
            if result.source_url and result.source_url not in [r.get('url') for r in resources]:
                resources.append({
                    'title': result.title,
                    'url': result.source_url,
                    'description': self._create_snippet(result.content)
                })

        if not resources:
            return "Additional resources can be found in the official documentation."

        resource_text = "Additional Resources:\n\n"
        for resource in resources[:3]:  # Limit to 3 resources
            resource_text += f"• {resource['title']}\n  {resource['url']}\n  {resource['description']}\n\n"

        return resource_text

    def _extract_quick_fixes(self, results: List[HybridSearchResult]) -> List[str]:
        """Extract quick fix solutions from troubleshooting content."""
        quick_fixes = []

        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['fix', 'solution', 'resolve', 'quick', 'simple']):
                # Extract sentences with solution keywords
                sentences = result.content.split('.')
                for sentence in sentences:
                    if any(keyword in sentence.lower() for keyword in ['fix', 'solution', 'try']):
                        if len(sentence.strip()) > 20:  # Avoid very short fixes
                            quick_fixes.append(sentence.strip())

        # Default quick fixes if none found
        if not quick_fixes:
            quick_fixes = [
        ...
        return quick_fixes[:3]  # Limit to 3 quick fixes

    def _extract_detailed_solutions(self, results: List[HybridSearchResult]) -> str:
        """Extract detailed solutions from troubleshooting content."""
        solutions = []

        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['solution', 'resolve', 'fix', 'troubleshoot']):
                solutions.append(self._create_snippet(result.content))

        if not solutions:
            return "Detailed solutions can be found by consulting the official documentation or community forums."

        return "\n\n".join(solutions[:2])  # Limit to 2 detailed solutions

    def _extract_common_causes(self, results: List[HybridSearchResult]) -> str:
        """Extract common causes of problems."""
        causes = []

        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['cause', 'reason', 'because', 'due to']):
                # Extract relevant sentences
                sentences = result.content.split('.')
                for sentence in sentences:
                    if any(keyword in sentence.lower() for keyword in ['cause', 'reason', 'because']):
                        if len(sentence.strip()) > 15:
                            causes.append(sentence.strip())

        if not causes:
            return "Common causes include configuration issues, dependency conflicts, or environment setup problems."

        return "\n".join(causes[:3])  # Limit to 3 causes

    def _extract_prevention_tips(self, results: List[HybridSearchResult]) -> str:
        """Extract prevention tips from content."""
        tips = []

        for result in results:
            content_lower = result.content.lower()
            if any(keyword in content_lower for keyword in ['prevent', 'avoid', 'best practice', 'tip']):
                sentences = result.content.split('.')
                for sentence in sentences:
                    if any(keyword in sentence.lower() for keyword in ['prevent', 'avoid', 'best']):
                        if len(sentence.strip()) > 15:
                            tips.append(sentence.strip())

        if not tips:
            return "Prevention tips: Follow best practices, keep dependencies updated, and monitor system health regularly."

        return "\n".join(tips[:3])  # Limit to 3 tips
