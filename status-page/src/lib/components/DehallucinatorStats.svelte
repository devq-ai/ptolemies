	...
	<div class="card-body">
		...
		<div class="stats stats-vertical lg:stats-horizontal shadow bg-black mb-4">
			<div class="stat">
				...
				<div class="stat-title text-white">Accuracy Rate</div>
				<div class="stat-value text-white">{dehallucinatorStats.accuracy_rate}%</div>
				<div class="stat-desc text-white">AI detection accuracy</div>
			</div>
			...
			<div class="stat">
				...
				<div class="stat-title text-white">False Positive</div>
				<div class="stat-value text-white">{dehallucinatorStats.false_positive_rate}%</div>
				<div class="stat-desc text-white">Error rate</div>
			</div>
			...
			<div class="stat">
				...
				<div class="stat-value text-white">{dehallucinatorStats.frameworks_supported}</div>
				<div class="stat-desc text-white">Supported</div>
			</div>
			...
			<div class="stat">
				...
				<div class="stat-title text-white">API Patterns</div>
				<div class="stat-value text-white">{dehallucinatorStats.api_patterns_detected.toLocaleString()}</div>
				<div class="stat-desc text-white">Detected patterns</div>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
			<div class="card bg-black shadow">
				<div class="card-body">
					<h3 class="card-title text-sm text-white">Performance</h3>
					<div class="space-y-2">
						<div class="flex justify-between text-xs">
							<span>Avg Time/File:</span>
							<span class="font-bold text-white">{analysisPerformance.avg_time_per_file}</span>
						</div>
						<div class="flex justify-between text-xs">
							<span>Memory Usage:</span>
							<span class="font-bold text-white">{analysisPerformance.memory_usage}</span>
						</div>
						<div class="flex justify-between text-xs">
							<span>Concurrent Files:</span>
							<span class="font-bold text-white">{analysisPerformance.concurrent_processing}</span>
						</div>
						<div class="flex justify-between text-xs">
							<span>Repo Scan:</span>
							<span class="font-bold text-white">{analysisPerformance.repository_scan_time}</span>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="card bg-black shadow">
				<div class="card-body">
					<h3 class="card-title text-sm text-white">Recent Activity</h3>
					<div class="space-y-2">
						<div class="flex justify-between text-xs">
							<span>Analyses Today:</span>
							<span class="font-bold text-white">{dehallucinatorStats.recent_analyses}</span>
						</div>
						<div class="flex justify-between text-xs">
							<span>Service Version:</span>
							<span class="font-bold text-white">v{dehallucinatorStats.service_version}</span>
						</div>
						<div class="flex justify-between text-xs">
							<span>Last Analysis:</span>
							<span class="font-bold text-white">{formatTimestamp(dehallucinatorStats.last_analysis).split(',')[1]}</span>
						</div>
					</div>
				</div>
			</div>
				...
				<tbody>
					{#each detectionCategories as category}
						<tr class="hover:bg-black">
							<td class="font-medium text-white">{category.category}</td>
							<td>
								<div class="badge text-white">{category.count.toLocaleString()}</div>
							</td>
							<td class="text-xs opacity-70 text-white">{category.description}</td>
							<td>
								<div class="badge text-white badge-sm">
		...
		<!-- Framework Coverage -->
		<div class="collapse collapse-arrow bg-black mb-4">
			<input type="checkbox" />
			<div class="collapse-title text-sm font-medium text-white">
		...
		<div class="alert bg-black text-white mt-4">
			<div class="flex-1">
				<div class="text-sm">
					...
					<code class="text-xs bg-black text-white px-1 rounded">python dehallucinator/ai_hallucination_detector.py target_script.py</code><br/>
					<strong>Repository Analysis:</strong>
					<code class="text-xs bg-black text-white px-1 rounded">python ai_hallucination_detector.py --repo /path/to/repo</code>
				</div>
			</div>
		</div>
	</div>
